...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    WeightedConcatDataset, ConcatDatasetEEG,
...
class PrintVisionEncoderLrCallback(TrainerCallback):
    """Debugging callback that logs vision-encoder learning rate during training."""

    def __init__(self, model, print_every=100, key_substr="vision_model"):
...
class ModelArguments:
    ...
    contrastive_method: Optional[str] = field(
        default=None,
        metadata={'help': 'soft loss or OT'}
    )
    resample_method: Optional[str] = field(
        default=None,
        metadata={'help': 'resample method'},
    )
    num_query_tokens: Optional[int] = field(
        default=32,
        metadata={'help': 'resample method'},
    )
    num_cquery_tokens: Optional[int] = field(
        default=1,
        metadata={'help': 'resample method'},
    )
    num_tquery_tokens: Optional[int] = field(
        default=1,
        metadata={'help': 'resample method'},
    )
    num_query_pool: Optional[int] = field(
        default=32,
        metadata={'help': 'resample method'},
    )
    use_fuse: Optional[bool] = field(
        default=False,
        metadata={'help': 'use fuse'},
    )
    # seed: int = field(
    #     default=0,
...
class DataTrainingArguments:
    ...
    train_meta_path: Optional[str] = field(
        default=None,
    ...
    normalize_type: Optional[str] = field(
        default='mean_std',
        metadata={'help': 'mean_std or z_score.'},
    )
    drop_channel: Optional[bool] = field(
        default=False,
        metadata={'help': 'Set to True to begin drop_channel.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        self.template_name = template_name      # internlm2-chat

        if "ctqformer_v2" in resample_method:
            num_c = len(ch_names)
            num_T = int((meta["num_image_token"] - 1) / num_c)
            self.num_image_token = (num_c * num_tquery_tokens) + (num_T * num_cquery_tokens) + 1
        else:
            if ds_name == "BCI2A":
                self.num_image_token = 1
            else:
                self.num_image_token = meta["num_image_token"]

        logger.info(f'[Dataset] num_image_token: {self.num_image_token}')
    ...
    def multi_modal_get_item(self, data_item):
        ...
        text_answer = data_item['conversations'][1]['value']
        answer_tokenizer = self.tokenizer(
            text_answer,
            return_tensors='pt',
            padding='max_length',
            max_length=20,
        )
        answer_input_ids = answer_tokenizer.input_ids
        answer_mask = answer_tokenizer.attention_mask
        image_path = data_item['EEG']

        # Load the image using tcs_loader if available, otherwise use PIL
        image = self.load_eeg(image_path)

        if self.normalize_type == "zscore":
            if self.ds_name in ["TUSL", "TUAB", "TUEV", "Workload", "BCI2A", "SEEDIV", "SleepEDF", "SHHS"]: 
                ...
                image = torch.from_numpy(image) 
        elif self.normalize_type == "mean_std":
            image = torch.from_numpy(image).float()
            mean = image.mean(dim=0)
            std = image.std(dim=0)
            image = (image - mean) / std
        elif self.normalize_type == "old":
        ...
        if self.drop_channel:
            ...
            drop_mask = torch.rand(num_channels) < self.drop_prob
            image[drop_mask] = 0  

        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically，false
        ...
        pixel_values = [image for image in images]
        ...
        ret = dict(
            ...
            text_answer=answer_input_ids[0],
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            try:
                ...
                if 'EEG' in data_item and len(data_item['EEG']) != 0:
                ...
                data_item = json.loads(self.raw_data[i])
                if 'EEG' in data_item:
                    if type(data_item['EEG']) == list:
                        eeg = [self.root + item for item in data_item['EEG']]
                        print(f'Failed to load eeg: {eeg}, the dataset is: {self.ds_name}')
                    else:
                        if data_item['EEG'].startswith('s3://'):
                            data_path = self.root + data_item['EEG']
                        else:
                            data_path = os.path.join(self.root, data_item['EEG'])
                        print(f'Failed to load eeg: {data_path}, the dataset is: {self.ds_name}')
...
def build_datasets(
    ...
    datasets = []
    lengths = []
    ds_collections = json.loads(open(data_args.train_meta_path).read())
    dataset_info = {}
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        dataset_info[ds_name] = {}
        dataset_info[ds_name]['ch_names'] = ds_collections[ds_name]['ch_names']
        dataset_info[ds_name]['task_caption'] = ds_collections[ds_name]['task_caption']
        dataset_info[ds_name]['num_image_token'] = ds_collections[ds_name]['num_image_token']
        ...
        dataset = LazySupervisedDataset(
            ...
            num_cquery_tokens=num_cquery_tokens,
            num_tquery_tokens=num_tquery_tokens,
    ...
    if data_args.use_data_resampling:
        ...
        train_dataset = ConcatDatasetEEG(datasets)
    datasets_index = train_dataset.get_index()
    test_dataset = []

    return train_dataset, test_dataset, datasets_index, dataset_info


def initialize_ctqformer_weights(model):
    for name, param in model.named_parameters():
        if name.startswith("ctqformer"):
            if "attn" in name: 
                if "weight" in name:
    ...
    if  hasattr(model, "router_manager"):
        for name, router in model.router_manager.routers.items():
            for module in router.modules():
                if isinstance(module, nn.Linear):
                    init.kaiming_uniform_(module.weight, nonlinearity='relu')
                    if module.bias is not None:
                        init.zeros_(module.bias)
    if hasattr(model, "time_router_manager"):
        mgr = model.time_router_manager
        if hasattr(mgr, "routers"):
            routers = mgr.routers.values()
        if hasattr(mgr, "router"):
            routers = [mgr.router]
        else:
            routers = [mgr]
        for router in routers:
            for module in router.modules():
                if isinstance(module, nn.Linear):
                    ...
                    if module.bias is not None:
                        init.zeros_(module.bias)
    if hasattr(model, "channel_router_manager"):
        mgr = model.channel_router_manager
        if hasattr(mgr, "routers"):
            routers = mgr.routers.values()
        if hasattr(mgr, "router"):
            routers = [mgr.router]
...
def initialize_routers(model, method="channel_router"):
    if method == "channel_router" and hasattr(model, "router_manager"):
        for name, router in model.router_manager.routers.items():
            for module in router.modules():
                if isinstance(module, nn.Linear):
                    init.kaiming_uniform_(module.weight, nonlinearity='relu')
...
def main():
    ...
    # setattr(data_args, 'data_batch_size', training_args.per_device_train_batch_size)
    ...
    train_dataset, test_dataset, datasets_index, dataset_info = build_datasets(
        data_args, tokenizer, tcs_loader,
        ...
        num_cquery_tokens=model_args.num_cquery_tokens,
        num_tquery_tokens=model_args.num_tquery_tokens,
        drop_channel=data_args.drop_channel,
    )

    if model_args.model_name_or_path is not None:
        logger.info('Loading InternVLChatModel...')
        config = InternVLChatConfig.from_pretrained(model_args.model_name_or_path)
        config.contrastive_method = model_args.contrastive_method
        config.use_fuse = model_args.use_fuse
        config.resample_method = model_args.resample_method
        config.num_query_tokens = model_args.num_query_tokens
        config.num_cquery_tokens = model_args.num_cquery_tokens
        config.num_tquery_tokens = model_args.num_tquery_tokens
        config.num_query_pool = model_args.num_query_pool
        ...
        model = InternVLChatModel.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config,ignore_mismatched_sizes=True)
    else:
    ...
    if model_args.mlp_path is not None:#None
    ...
    # num_image_token must equal (num_c * num_tquery_tokens) + (num_T * num_cquery_tokens) + 1
    # for ctqformer_v2 variants. Hardcoded here to 61 for the 8B / 10-dataset configuration.
    model.num_image_token = 61
    ...
    #     min_dynamic_patch=data_args.min_dynamic_patch, max_dynamic_patch=data_args.max_dynamic_patch,
    #     normalize_type=data_args.normalize_type, resample_method=config.resample_method, num_query_tokens=config.num_query_tokens, num_cquery_tokens=config.num_cquery_tokens, num_tquery_tokens=config.num_tquery_tokens, drop_channel=data_args.drop_channel)
    ...
    if model_args.unfreeze_lm_head:
        # Unfreeze the last 3 decoder layers to allow partial LLM gradient flow.
        layers = model.language_model.base_model.model.model.layers
        num_layers = len(layers)
        for layer in layers[num_layers - 3:]:
    ...
    if "ctqformer" in config.resample_method:
        initialize_ctqformer_weights(model)
    ...
    training_args.distributed_data_parallel = True
    training_args.dataset_range = datasets_index
