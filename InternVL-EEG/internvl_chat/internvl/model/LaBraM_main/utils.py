...
from .data_processor.instr_dataset import ShockDataset
import pickle
import sklearn
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None, save_ckpt_freq=1):
    ...
    if not getattr(args, 'enable_deepspeed', False):
        checkpoint_paths = [encoder_checkpoint / 'checkpoint.pth']
        if epoch == 'best':
            checkpoint_paths = [encoder_checkpoint / ('checkpoint-%s.pth' % epoch_name),]
        elif (epoch + 1) % save_ckpt_freq == 0:
            checkpoint_paths.append(encoder_checkpoint / ('checkpoint-%s.pth' % epoch_name))
        ...
        model.save_checkpoint(save_dir=args.encoder_checkpoint, tag="checkpoint-%s" % epoch_name, client_state=client_state)           
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None):
    ...
    # if 0:
        ...
        if args.auto_resume and len(args.resume) == 0:
            all_checkpoints = glob.glob(os.path.join(encoder_checkpoint, 'checkpoint.pth'))
            if len(all_checkpoints) > 0:
                args.resume = os.path.join(encoder_checkpoint, 'checkpoint.pth')
            else:
                all_checkpoints = glob.glob(os.path.join(encoder_checkpoint, 'checkpoint-*.pth'))
                latest_ckpt = -1
                ...
                if latest_ckpt >= 0:
                    args.resume = os.path.join(encoder_checkpoint, 'checkpoint-%d.pth' % latest_ckpt)
        ...
        if args.auto_resume:
            all_checkpoints = glob.glob(os.path.join(encoder_checkpoint, 'checkpoint-*'))
            latest_ckpt = -1
            for ckpt in all_checkpoints:
                t = ckpt.split('-')[-1].split('.')[0]
                if t.isdigit():
                    latest_ckpt = max(int(t), latest_ckpt)
            if latest_ckpt >= 0:
                args.resume = os.path.join(encoder_checkpoint, 'checkpoint-%d' % latest_ckpt)
                print("Auto resume checkpoint: %d" % latest_ckpt)
                _, client_states = model.load_checkpoint(args.encoder_checkpoint, tag='checkpoint-%d' % latest_ckpt)
...
def create_ds_config(args):
    Path(args.encoder_checkpoint).mkdir(parents=True, exist_ok=True)
    with open(os.path.join(args.encoder_checkpoint, "latest"), mode="w") as f:
        pass

    args.deepspeed_config = os.path.join(args.encoder_checkpoint, "deepspeed_config.json")
...
def build_pretraining_dataset(datasets: list, time_window: list, stride_size=200, start_percentage=0, end_percentage=1):
    ...
    for dataset_list, window_size in zip(datasets, time_window):
        ...
        inputs, outputs = dataset.get_conversations()
        input_list.append(inputs)
        output_list.append(outputs)
    return shock_dataset_list, ch_names_list, input_list, output_list
...
def prepare_SEED_dataset(root):
    # set random seed
    seed = 12345
    np.random.seed(seed)

    SEED_files = os.listdir(root)
    print(len(SEED_files))

    # prepare training and test data loader
    SEED_dataset = SEEDLoader(root, SEED_files)
    return SEED_dataset
